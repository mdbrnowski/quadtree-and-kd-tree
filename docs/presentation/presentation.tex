...
\begin{frame}{Drzewa czwórkowe -- analiza złożoności}
    \begin{lemma}[wysokość drzewa czwórkowego]
        Niech wszystkie punkty ze zbioru $P$ będą zawarte w kwadracie o boku $1$. Przez $\phi$ oznaczymy najmniejszą odległość między pewnymi dwoma punktami. Wysokość $h$ drzewa czwórkowego dla zbioru $P$ jest rzędu $\sO(\log \phi^{-1})$.

        \pause Można zauważyć, że $h$ jest $\Omega(\log n)$ (ograniczona asymptotycznie od dołu; jeśli punkty w $P$ są rozłożone w równomierną siatkę, to $h \in \Theta(\log n)$).
    \end{lemma}
    ...
    \onslide<3->{
        ...
        \item zapytania: \onslide<5->{$\sO(hk)$.}
        \end{enumerate}
    }
\end{frame}
